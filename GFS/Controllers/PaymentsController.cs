...
namespace GFS.Controllers
{
    public class PaymentsController : Controller
    {
        ...
        // GET: Payments
        public ActionResult Index(string searchString, string startdate, string enddate,string criteria)
        {
            ...
            {
                pay = (from d in pay
                       where Convert.ToInt32(DateTime.Now.Day) - Convert.ToInt32(d.datePayed.Day) / 30 > 10
                       select d).ToList();
            }
            else if(criteria=="Over 30 Days")
            {
                pay = (from d in pay
                       where Convert.ToInt32(DateTime.Now.Day) - Convert.ToInt32(d.datePayed.Day) / 30 > 5
                       select d).ToList();
            }
            return View(pay);
        }
            ...
            if(Session["fullname"]==null)
            {
                ...
                if (payment.emailSlip==true)
                {
                    try
                    {
                        ...
                        string subject_for = "G.F.S Payment On "+DateTime.Now.ToString();
